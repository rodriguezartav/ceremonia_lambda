...
Lambda.prototype.setup = function () {
  ...
  this._createSampleFile('.envs');
  this._createSampleFile('.config');
  this._createSampleFile('event.json');
  this._createSampleFile('context.json');
  this._createSampleFile('package.json');
  this._createSampleFile('index.js');
...
Lambda.prototype._codeDirectory = function (program) {
  ...
  return '../_temp/' + program.functionName + '-' + epoch_time;
};

Lambda.prototype._cleanDirectory = function (codeDirectory, callback) {
  console.log( codeDirectory );
  exec('rm -rf ' + codeDirectory, function (err) {
    if (err) {
      throw err;
    }

    mkdirp(codeDirectory, function(err) {
      if (err) {
        throw err;
      }
...
Lambda.prototype.package = function (program) {
  ...
  if (!program.packageDirectory) {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        console.log('=> Creating package directory', program.packageDirectory);
  ...
  _this._archive(program, function (err, buffer) {
    ...
    var basename = program.functionName;
